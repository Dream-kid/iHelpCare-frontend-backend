...
export default function DetailsCaregiverModal({ detailsCaregiverModal, setDetailsCaregiverModal }) {
  // fetch details API's data
  const [loadingDetails, errorDetails, dataDetails] = useFetchData(
    `/admin/care-giver/details?id=${detailsCaregiverModal?.id}`
  );

  // function to handel close modal
  const handleCloseModal = () => {
    setDetailsCaregiverModal((prevState) => ({ ...prevState, open: false, id: null }));
  };

  return (
    <Modal
      className='relative my-5 lg:my-10'
      open={detailsCaregiverModal?.open}
      // prettier-ignore
      title={(
        <div>
...
DetailsCaregiverModal.propTypes = {
  detailsCaregiverModal: PropTypes.shape({
    open: PropTypes.bool.isRequired,
    id: PropTypes.number.isRequired,
  }).isRequired,
  setDetailsCaregiverModal: PropTypes.func.isRequired,
};
