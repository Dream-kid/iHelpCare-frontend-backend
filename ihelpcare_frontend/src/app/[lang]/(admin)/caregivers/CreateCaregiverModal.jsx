...
export default function CreateCaregiverModal({
  createCaregiverModalOpen,
  setCreateCaregiverModalOpen,
  ...
  useEffect(() => {
    if (success) {
      openNotificationWithIcon(
        'success',
        'SUCCESS',
        success?.message || 'Caregiver created successfully!'
      );
      setDataFetchAgain((prevState) => !prevState);
      setCreateCaregiverModalOpen(false);
      form.resetFields();
    }
  }, [success]);
  ...
  const onFinish = (values) => {
    postData('/admin/create/care-giver', {
    ...
    >
      ...
      >
        ...
        <div className='mt-10 flex flex-row items-center justify-between space-x-5'>
          ...
          <Form.Item className='flex-1'>
            <Button
              onClick={() => setCreateCaregiverModalOpen(false)}
              type='default'
              size='large'
              block
            >
...
CreateCaregiverModal.propTypes = {
  createCaregiverModalOpen: PropTypes.bool.isRequired,
  setCreateCaregiverModalOpen: PropTypes.func.isRequired,
